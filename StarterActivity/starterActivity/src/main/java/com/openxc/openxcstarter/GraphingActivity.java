...
public class GraphingActivity extends Activity implements OnItemSelectedListener  {
	...
	LineGraphSeries<DataPoint> ChargeSeries = new LineGraphSeries<DataPoint>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<Double> listTemp = (ArrayList<Double>) getIntent().getSerializableExtra("listTemp");
		ArrayList<Double> listCharge = (ArrayList<Double>) getIntent().getSerializableExtra("listCharge");
		...
		if(listTemp != null){
			for (int i = 0; i < listTemp.size(); i++){
				AcCompPowSeries.appendData(new DataPoint(i,listTemp.get(i)), true, maxPoints);
			}
		}
		if(listCharge != null){
			for (int i = 0; i < listCharge.size(); i++){
				AcCompPowSeries.appendData(new DataPoint(i,listCharge.get(i)), true, maxPoints);
			}
		}
	...
	public void onItemSelected(AdapterView<?> parentView, View v, int position,
		...
		else if(canSelect.getSelectedItem().toString().equals("Temperature")){
		...
		else if(canSelect.getSelectedItem().toString().equals("Charging Status")){
			graph.addSeries(ChargeSeries);
			graph.setTitle("Charging Status");
			Charge = true;
			//Toast.makeText(getApplicationContext(), "Relative Drive Power", Toast.LENGTH_SHORT).show();
